...
import * as fns from "../calculator.js";
...
/*describe("Update Display", () => {
  test("Updates text content of display element", () => {
    document.body.innerHTML = '<div id="display"><p id ="output"></p></div>';
    const value = "TEST";

    expect(document.getElementById("output").textContent).toBe("");
    updateDisplay(value);
    expect(document.getElementById("output").textContent).toBe(value);
  });
});

describe("AddButtonClickEvent", () => {
  jest.mock("../addButtonClickEvents");

  test("Adds click event listener to buttons", () => {
    // Set up document body
    document.body.innerHTML =
      '<div class="buttons"> <button type="button">1</button><button type="button">2</button><button type="button">3</button></div>';

      
    const addButtonClickEvents = require
    addButtonClickEvent();

    let buttons = document.getElementsByTagName("button");

    for (let i = 0; i < buttons.length; i++) {
      expect(buttons[i].getAttribute(data - clickEvent)).toBe("true");
    }
  });
}); */
